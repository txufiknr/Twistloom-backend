...
import type { StoryThread, ThreadClue, NewThread, UpdateThread, AddThreadClue } from "../types/story-thread.js";
...
export function extractStateDelta(params: {
  ...
  const stateDelta: StateDelta = {
    ...
    newCharacters: generation.newCharacters,
    updatedCharacters: generation.updatedCharacters,
    relationshipUpdates: generation.relationshipUpdates,
    placeConnections: generation.placeConnections,
    newPlaces: generation.newPlaces,
    updatedPlaces: generation.updatedPlaces,
    newThreads: generation.newThreads,
    updateThreads: generation.updateThreads,
    addClues: generation.addClues,
    closeThreads: generation.closeThreads,
...
export function applyStateDelta(baseState: StoryState, stateDelta: StateDelta, scene?: StoryScene): StoryState {
  ...
  processCharacterUpdates(newState, newCharacters, updatedCharacters, relationshipUpdates, scene?.placeId);
  processPlaceUpdates(newState, newPlaces, updatedPlaces, placeConnections, scene, previousPlaceId);
  processThreadUpdates(newState, newThreads, updateThreads, addClues, closeThreads);
...
export async function advanceStoryState(state: StoryState, actionedPage: Pick<CandidateGenerationPage, 'page' | 'actions' | 'action' | 'momentum' | 'sceneType'>): Promise<StoryState> {
  ...
  const selectedIndex = allActions.findIndex((a: Action) => a.text === action.text);
 ...
 *
 * @param state - Mutable story state
 * @param newThreads - New threads to create
 * @param updateThreads - Updates to existing threads
 * @param addClues - Clues to add to existing threads
 * @param closeThreads - Threads to close/resolve
 */
export function processThreadUpdates(state: StoryState, newThreads?: NewThread[], updateThreads?: UpdateThread[], addClues?: AddThreadClue[], closeThreads?: string[]): void {
  ...
  if (updateThreads?.length) {
    for (const update of updateThreads) {
  ...
  if (closeThreads?.length) {
    for (const thread of closeThreads) {
