...
export const books = pgTable(
  "books",
  {
    ...
    // generatedKeywords: text('generated_keywords').generatedAlwaysAs(
    //   // BTRIM strips the '["' and '"]' generated by postgres text-casting the jsonb
    //   sql`btrim(${sql.identifier('keywords')}::text, '[]"')`
    // ),

    // keywordsText: text("keywords_text"),
    // TODO: you said this is "even better", so I don't need `keywords` and `keywordsText` anymore in favor of only this one?
    // keywordsArray: text('keywords_array').array(),

    // TODO: or use this one?
    // // Auto-generated to eliminate heavy CTE calculations
    // generatedKeywordsArray: text('generated_keywords_array').generatedAlwaysAs(
    //   sql`ARRAY(
    //     SELECT jsonb_array_elements_text(keywords)
    //   )`
    // ),

    // TODO: or you said you would add 3 additional columns:
    // - generatedKeywords: text(...) // display / AI prompt purposes
    // - keywordsArray: text[] // generated stored
    // - keywordsTsv: tsvector // generated stored?
  ...
  (t) => [
    ...
    // TODO: create index on keywords_text
    // CREATE INDEX books_keywords_text_tsv_idx
    // ON books
    // USING gin (
    //   to_tsvector('simple', keywords_text)
    // );
  ]
);
